...
## A. 日本語ロケールで日本語表示（2026-05 整理後）

### アプリ（`ja.lproj` に日本語があるもの）

| # | キー | 日本語表示 |
|---|------|------------|
| 1 | `preset.library.slot_limit_notice` | 保存できるのは最大 %d 件までです。 |
| 2 | `onboarding.title` | JChordへようこそ |
| 3 | `onboarding.subtitle` | GarageBandと連携して演奏を始めましょう |
| 4 | `onboarding.garageband.title` | GarageBand の設定 |
| 5–9 | `onboarding.garageband.step1` … `step5` | （各ステップ文） |
| 10 | `onboarding.continue` | JChord をはじめる |
| 11 | `onboarding.close` | 閉じる |
| 12 | `settings.midi_commands.accessibility` | Note On/Off 以外の MIDI を送信 |
| 13 | `settings.midi_command.midiStop` | MIDI Stop（シーケンス停止） |
| 14 | `settings.midi_command.sustainOff` | サステイン Off（現在 CH） |
| 15 | `settings.midi_command.panicCurrentChannel` | Panic（現在 CH） |
| 16 | `settings.midi_command.panicAllChannels` | Panic（全 CH + MIDI Stop） |
| 17 | `pad_editor.section.expanded.accessibility` | 展開 |
| 18 | `pad_editor.section.collapsed.accessibility` | 折りたたみ |

### 削除済み（`LOCALIZATION_MESSAGES_re.md` の X / X?）

| キー | 対応 |
|------|------|
| `preset.picker.plan_test_label`（ja） | 削除 → `Plan (test)` |
| `jcstore.published_at` | en/ja とも削除（未使用） |
| `jcstore.open_web` | en/ja とも削除（未使用） |
| `preset.library.default_title`（ja） | 削除 → `No Set` |
| `preset.library.empty`（ja） | 削除 → 英語 empty 文 |
| `jcstore.replace_hint`（ja） | 削除 → 英語 hint |
| `preset.io.new_blank.accessibility`（ja） | 削除 |
| `preset.io.duplicate.accessibility`（ja） | 削除 |
| `preset.jazz_fusion` 等（ja の重複） | 削除 → en のみ |

### Web（`web/jcstore/index.html`）

| # | 出所 | 日本語表示 |
|---|------|------------|
| 19 | Web `tagline` | 公式プリセットカタログ（デモ） |
| 20 | Web イントロ段落 | （`web/jcstore/index.html` 直書き） |
...
| 48 | `preset.jazz_fusion` | Jazz Fusion |
| 49 | `preset.bossa_nova` | Bossa Nova |
| 50 | `preset.progressive_rock` | Progressive |
| 51 | `preset.standard_jazz` | Standard Jazz |
| 52 | `preset.city_pops` | City Pops |
